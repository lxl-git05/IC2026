...
module NPC(clk,rst,NPCOp,Offset12,Offset20,PC,rs,PCA4, NPC);
    ...
    // rs延迟,rs来自ID,所以只需要延迟1拍
    Delay #(32) Delay_rs_inst (
        .clk(clk),
        .rst(rst),
        .in(rs),
        .delay_num(2'b01),  // 1周期延迟
    ...
    always@(*) begin
        case(NPCOp)
            ...
            `NPC_rs       : NPC = (rs_EX + offset_JalR_EX ) & 32'hffff_fffe;  // jalr                                           // jalr
